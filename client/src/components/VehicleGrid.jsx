...
export const VehicleGrid = forwardRef(function VehicleGrid(
  ...
  if (loading) return <span className="text-muted text-xs px-2">{t('common.loading')}</span>;

  return (
    <>
      {loadError && (
        <div className="w-full mb-3 rounded-lg border border-red-500/40 bg-red-500/15 px-3 py-2 text-xs text-red-200">
          {loadError}
        </div>
      )}
      <div
      ...
      >
        ...
        {!hideAddButton && (
          <button
            ...
            className="w-14 h-14 border border-dashed border-border text-muted hover:border-accent hover:text-accent flex items-center justify-center text-2xl shrink-0 transition-colors bg-surfaceContainer"
          >
            +
          </button>
        )}
