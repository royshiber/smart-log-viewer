    ...
    <div className="relative w-full" ref={panelRef}>
      ...
      <div className="mt-1.5 flex flex-col gap-1">
        {comparisons.map((comp, i) => (
          <div
            key={comp.id}
            className="flex items-center gap-2 px-2 py-1.5 rounded-xl border text-xs shrink-0"
            style={{ borderColor: `${COMP_COLORS[i % COMP_COLORS.length]}80`, background: `${COMP_COLORS[i % COMP_COLORS.length]}18` }}
          >
            <div className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: COMP_COLORS[i % COMP_COLORS.length] }} />
            <div className="min-w-0 flex-1">
              <div className="text-gray-100 truncate font-medium">{comp.vehicleName}</div>
              <div className="text-gray-400 truncate">{comp.logName}</div>
            </div>
